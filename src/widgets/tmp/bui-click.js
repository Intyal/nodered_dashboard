...
export class BuiClick extends BUIBaseWidget {
	...
	constructor() {
		...
		this.log = document.getElementById('bui_2a9c73dbf0e33e78');
	}
	...
	connectedCallback() {
		...
		if (this._targetElement) {
			this._targetElement.style.touchAction = 'none';
		}
	}
	...
	_handlePointerDown(event) {
		...
		this._pressTimer = setTimeout(() => {
			...
			this.pressType(event, 'longPress');
		}, this.duration);
	...
	_handleGlobalPointerUp(event) {
		...
		if (!this._isLongPress) {
			...
			this.pressType(event, 'shortPress');
		}

		this.pressType(event, 'up');
	}
	...
	pressType(event, type) {
		...
		if (this.event === type) {
			...
			//if (data) {
				//console.log(data);
				this._switchAction(this.action, data, JSON.parse(this.params || '{}'));
			//}
		}
	}

	_switchAction(action, data, params) {
		try {
